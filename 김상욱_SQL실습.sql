...
-- select 학생.학번, 학생.이름, mid(학생.이름, 1, 1) as 성 from 학생;
select 학생.학번, 학생.이름, left(학생.이름, 1) as 성
from 학생;
...
select 수강신청.수강신청번호, 수강신청.학번, 수강신청.날짜
from 수강신청
where year(수강신청.날짜) = 2018 and month(수강신청.날짜) = 03;
...
select 학생.학번, right(학생.이름, 2) as 이름
from 학생;
...
select 수강신청.학번, 수강신청.날짜
from 수강신청
where month(수강신청.날짜) = 3 and day(수강신청.날짜) = 1;
-- where 수강신청.날짜 = date_format('0301', '%m%d');

-- 46 <학생> 테이블에서 
-- 아래 그림과 같이 '학번', '이름', 학과'와 
-- 컴퓨터정보학과인지 다른 학과인지를 출력하는 SQL문을 작성하시오. (복습)
...
select 학생.학번, 학생.이름, if(학과.학과명 = "컴퓨터정보학과", "컴퓨터정보학과", "타과") as 비고
from 학생, 학과
where 학생.학과 = 학과. 학과번호;
...
-- select count(교수.전화번호) as '교수 수' from 교수;
...
select 과목.담당교수, count(과목.과목명) as 과목수, sum(과목.학점) as 학점수
from 과목
group by 과목.담당교수;
...
select count(distinct 수강신청내역.과목번호) as 과목수, avg(수강신청내역.평점) as 평균평점
from 수강신청내역
where 수강신청내역.수강신청번호 = '1810001';

-- 57 <수강신청내역> 테이블에서 
-- 아래와 같이 과목별로 수강자 수를 읽어오는(과목번호, 수강자 수) SQL문을 작성하시오.
select 수강신청내역.과목번호, count(*) as '수강자 수'
from 수강신청내역
...
select 수강신청내역.과목번호, count(*) as '수강자 수', avg(수강신청내역.평점) as 평균평점
from 수강신청내역
where 수강신청내역.평점 <> -1
group by 수강신청내역.과목번호;
...
from 수강신청내역, 과목
where 과목.과목번호 = 수강신청내역.과목번호 and 수강신청내역.수강신청번호 = '1810001' or 수강신청내역.수강신청번호 = '1610001';

-- 65 <수강신청> 테이블에서 
-- 2018 년도 수강신청에 대하여 수강신청번호, 신청자 학번, 신청자 이름을 출력하는 SQL 문을 작성하시오.
select 수강신청.수강신청번호, 학생.학번, 학생.이름
from 수강신청, 학생
where 학생.학번 = 수강신청.학번 and year(수강신청.날짜) = 2018
order by 수강신청.수강신청번호;

-- 66 학번이 '1801001'인 학생이 수강 신청한 수강신청번호, 과목번호, 과목명을 출력하는 SQL 문을 작성하시오.  (복습)
select 수강신청내역.수강신청번호, 수강신청내역.과목번호, 과목.과목명
from 수강신청내역, 수강신청, 과목
where 과목.과목번호 = 수강신청내역.과목번호 
		and 수강신청.수강신청번호 = 수강신청내역.수강신청번호
        and 수강신청내역.수강신청번호 = '1810001';
...
select count(*) as '과목수', sum(수강신청내역.평점) as '평점합계'
from 수강신청내역, 과목
where 과목.과목번호 = 수강신청내역.과목번호 and 수강신청내역.수강신청번호 = '1810001';

-- 69 수강신청 별로 신청 학생의 학번, 이름, 평균 평점(평점합계/과목수)를 출력하는 SQL 문을 작성하시오. (복습)
-- (단, 학점이 부여된 수강신청만 출력한다)  

select 수강신청.수강신청번호, 수강신청.학번, 학생.이름, sum(평점)/count(수강신청내역.과목번호) as 평균평점
...
-- 70 모든 학생에 대하여 학번, 학과번호, 이름, 소속 학과 이름을 출력하는 SOL 문을 작성하시오. (중복)

-- 71 <수강신청내역> 테이블에서 
-- 수강신청번호 '1810001'와 '1610001'에 대하여 수강신청번호와 수강 과목번호, 수강 과목명을 출력하는 SQL 문을 작성하시오. (중복)

-- 72 <수강신청> 테이블에서 2018 년도 수강신청에 대하여 수강신청번호, 신청자 학번, 신청자 이름을 출력하는 SQL 문을 작성하시오. (중복)

-- 73 학번이 '1801001'인 학생이 수강 신청한 수강신청번호, 과목번호, 과목명을 출력하는 SQL 문을 작성하시오. (중복)

-- 74 사번이 '1000004'인 교수와 같은 학과에 소속되어 있는 교수의 사번, 학과, 이름을 출력하시오. (복습)
-- 결과값은 나왔지만, 교수의 사번 조건으로 교수의 학과 분류가 가능할까?
select 교수.사번, 교수.학과, 교수.이름 
from 교수
where 학과 = ( select 학과
				from 교수
				where 교수.사번 = '1000004');

-- 75 학생별로 학번, 이름, 수강 신청한 과목 수, 평균 평점을 출력하시오. (단, 학점이 부여된 수강 신청만 출력한다)  (중복)
